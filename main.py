...
class SheetsHelper:
    ...
    def _prepare_worksheet_headers(self, worksheet, title_name):
        ...
        # Правило валідації: галочка/хрестик (булеве значення)
        # https://gspread.readthedocs.io/en/latest/api/models/worksheet.html#gspread.worksheet.Worksheet.set_data_validation
        # Застосовуємо до всіх колонок статусу; починаючи з 4-го рядка
        if status_cols:
            # Формуємо діапазони для оновлення (наприклад, 'C4:C1000', 'F4:F1000'...)
            # ПРИМІТКА: Gspread set_data_validation не підтримує булеві значення; тому використовуємо gspread.worksheet.Worksheet.set_data_validation
            # для встановлення правила 'List of items' ['✅', '❌']
            # АБО gspread.worksheet.Worksheet.set_data_validation для 'Checkbox'
            
            # Спрощений варіант: встановлення правила валідації для першої 1000 рядків
            for col_index in status_cols:
                ...
                # Встановлюємо правило "List of items" з галочкою та хрестиком
    ...
    def add_chapters(self, title_name, chapter_numbers, telegram_tag, nickname):
        ...
        try:
            ...
            for chapter_number in chapters_to_add:
                ...
                # Додаємо дані для Публікації (ТІЛЬКИ Статус='❌')
                new_row_data.append('❌') 
...
# В цілях економії місця я опускаю незмінені функції тут; але вони є в повному коді;
    ...
    def update_chapter_status(self, title_name, chapter_number, role_name, status_char, nickname, telegram_tag):
        ...
        try:
            ...
            # ВИПРАВЛЕННЯ: Логіка для Публікації
            if role_key == PUBLISH_COLUMN_BASE:
                ...
                nick_col_index = None # Немає ніка для Публікації
                date_col_index = None # Немає дати для Публікації
            else:
                try:
                    ...
                    return f"❌ Помилка: Колонка для ролі '{role_key}' не знайдена в заголовках; Можливо; ви не встановили команду з Бетою;?"
...
async def handle_team_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('awaiting_team_input') and 'setting_team_for_title' in context.user_data:
        ...
        # Завершення: Вивід команди
        return

# --- MAIN RUNNER (Залишається без змін) ---
...
async def run_bot():
    ...
    application = ApplicationBuilder().token(TELEGRAM_BOT_TOKEN).bot_data({'sheets_helper': sheets_helper}).build()
    ...
    application.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), handle_team_input))

    # Запуск
    if WEBHOOK_URL and 'render.com' in WEBHOOK_URL:
        # Використання Webhook (для Render/інших PaaS)
        WEBHOOK_PORT = int(os.environ.get("PORT", "8443"))
        
        # Встановлення Webhook
        await application.bot.set_webhook(url=WEBHOOK_URL)
        logger.info(f"Встановлено Webhook на {WEBHOOK_URL}")

        # Запуск веб-сервера
        # Замість application.run_webhook; створюємо свій HTTP-сервер
        app = web.Application()
        # Створення обробника Webhook
        async def webhook_handler(request):
            if request.match_info['token'] == TELEGRAM_BOT_TOKEN:
                # Отримуємо JSON-дані
                data = await request.json()
                # Створюємо об'єкт Update та обробляємо його
        ...
        app.add_routes([
            web.post(f'/{TELEGRAM_BOT_TOKEN}', webhook_handler),
        ])
        ...
        logger.info(f"Запуск AIOHTTP сервера на 0.0.0.0:{WEBHOOK_PORT}")
        await site.start()
        
        # Запобігання виходу з функції (нескінченний цикл очікування)
        await asyncio.Event().wait()
        
    else:
        # Використання Long Polling (для локального запуску)
